...
func buildAllTestCases(testDate, lastWeek, lastMonth string) []apiTestCase {
	return []apiTestCase{
		...
		{"daily_share", "基础数据", "每日股本", "120积分",
			map[string]string{"ts_code": "000001.SZ", "start_date": lastWeek, "end_date": testDate}, "ts_code,trade_date"},
		{"stk_limit_l", "基础数据", "ST股票列表", "120积分",
			map[string]string{}, "ts_code,name"},
		{"stk_namechange", "基础数据", "股票曾用名(新)", "120积分",
			map[string]string{"ts_code": "000001.SZ"}, "ts_code,name"},
		{"stk_hsgt", "基础数据", "沪深港通股票列表", "120积分",
		...
		{"stk_mins_rt", "行情数据", "实时分钟行情", "120积分",
			map[string]string{"ts_code": "000001.SZ", "freq": "5min"}, "ts_code,trade_time,close"},
		{"stk_daily_rt", "行情数据", "A股实时日线", "120积分",
			map[string]string{"ts_code": "000001.SZ"}, "ts_code,close"},
		{"stk_daily_rt_mins", "行情数据", "A股实时分钟-日累计", "120积分",
			map[string]string{"ts_code": "000001.SZ"}, "ts_code,close"},
		{"weekly_adj", "行情数据", "周线复权行情", "120积分",
			map[string]string{"ts_code": "000001.SZ", "start_date": lastMonth, "end_date": testDate}, "ts_code,trade_date,close"},
		{"monthly_adj", "行情数据", "月线复权行情", "120积分",
			map[string]string{"ts_code": "000001.SZ", "start_date": lastMonth, "end_date": testDate}, "ts_code,trade_date,close"},
		{"ggt_daily", "行情数据", "港股通每日成交统计", "120积分",
			map[string]string{"start_date": testDate, "end_date": testDate}, "trade_date,gg_amount"},
		{"ggt_monthly", "行情数据", "港股通每月成交统计", "120积分",
			map[string]string{"start_date": lastMonth, "end_date": testDate}, "month,gg_amount"},
		{"stk_surv", "行情数据", "备用行情", "120积分",
			map[string]string{"ts_code": "000001.SZ"}, "ts_code,trade_date,close"},
		...
		{"margin_underlying", "参考数据", "融资融券标的", "2000积分",
			map[string]string{"exchange_id": "SSE"}, "ts_code,exchange_id"},
		{"stk_limit_step", "参考数据", "个股异常波动", "2000积分",
			map[string]string{"ts_code": "000001.SZ"}, "ts_code,trade_date"},
		{"stk_limit_step_severe", "参考数据", "个股严重异常波动", "2000积分",
			map[string]string{"ts_code": "000001.SZ"}, "ts_code,trade_date"},
		{"exchange_focus", "参考数据", "交易所重点提示", "2000积分",
		...
		{"margin_underlying", "两融数据", "融资融券标的", "2000积分",
			map[string]string{"exchange_id": "SSE"}, "ts_code,exchange_id"},
		{"sec_daily", "两融数据", "转融券交易汇总", "2000积分",
			map[string]string{"start_date": testDate, "end_date": testDate}, "trade_date,exchange"},
		{"sec_transfer", "两融数据", "转融资交易汇总", "2000积分",
			map[string]string{"start_date": testDate, "end_date": testDate}, "trade_date,exchange"},
		...
		{"hk_hold_detail", "特色数据", "中央结算系统持股明细", "2000积分",
			map[string]string{"code": "000001", "trade_date": testDate}, "code,trade_date"},
		{"hsgt_hold_stock", "特色数据", "沪深股通持股明细", "2000积分",
			map[string]string{"ts_code": "000001.SZ", "start_date": testDate, "end_date": testDate}, "ts_code,trade_date"},
		...
		{"stk_auction_c", "特色数据", "收盘集合竞价数据", "2000积分",
			map[string]string{"trade_date": testDate}, "ts_code,trade_date"},
		{"magic_nine_turn", "特色数据", "神奇九转指标", "2000积分",
			map[string]string{"ts_code": "000001.SZ", "start_date": testDate, "end_date": testDate}, "ts_code,trade_date"},
		{"ah_comparison", "特色数据", "AH股比价", "2000积分",
		...
		{"sw_index_daily", "指数专题", "申万实时行情", "120积分",
		...
		{"ci_index_daily", "指数专题", "中信行业成分", "120积分",
		...
		{"fut_weekly", "期货数据", "期货周月线行情", "2000积分",
			map[string]string{"ts_code": "M.DCE", "start_date": lastMonth, "end_date": testDate}, "ts_code,trade_date,close"},
		{"fut_mins", "期货数据", "期货历史分钟", "2000积分",
			map[string]string{"ts_code": "M.DCE", "freq": "5min", "start_date": testDate + " 09:00:00", "end_date": testDate + " 10:00:00"}, "ts_code,datetime,close"},
		{"fut_mins_rt", "期货数据", "期货实时分钟", "2000积分",
		...
		{"fut_tick", "期货数据", "历史Tick行情", "2000积分",
			map[string]string{"ts_code": "M.DCE", "trade_date": testDate}, "ts_code,datetime,last_price"},
		{"fut_holding", "期货数据", "每日持仓排名", "2000积分",
			map[string]string{"ts_code": "M.DCE", "trade_date": testDate}, "ts_code,trade_date"},
		{"fut_mapping", "期货数据", "期货主力与连续合约", "2000积分",
			map[string]string{"ts_code": "M.DCE"}, "ts_code,mapping_ts_code"},
		{"index_daily_nh", "期货数据", "南华期货指数行情", "2000积分",
			map[string]string{"ts_code": "NH0100", "start_date": testDate, "end_date": testDate}, "ts_code,trade_date,close"},
		{"fut_week_report", "期货数据", "期货主要品种交易周报", "2000积分",
			map[string]string{}, "ts_code,trade_date"},
		{"fut_limit", "期货数据", "期货合约涨跌停价格", "2000积分",
			map[string]string{"ts_code": "M.DCE", "trade_date": testDate}, "ts_code,trade_date"},

		// ══════════════════════════════════════════════════════════════
		// 现货数据（2）
		// ══════════════════════════════════════════════════════════════
		{"spot_basic", "现货数据", "上海黄金基础信息", "2000积分",
			map[string]string{}, "ts_code,name"},
		{"spot_daily", "现货数据", "上海黄金现货日行情", "2000积分",
		...
		{"cb_top10_holders", "债券专题", "可转债十大持有人", "120积分",
		...
		{"hk_daily_basic", "港股数据", "港股实时日线", "120积分",
			map[string]string{"ts_code": "00700.HK"}, "ts_code,close"},
		{"hk_adj", "港股数据", "港股复权行情", "120积分",
			map[string]string{"ts_code": "00700.HK", "start_date": testDate, "end_date": testDate}, "ts_code,trade_date,close"},
		{"hk_adj_factor", "港股数据", "港股复权因子", "120积分",
		...
		{"us_adj", "美股数据", "美股复权行情", "120积分",
			map[string]string{"ts_code": "AAPL", "start_date": testDate, "end_date": testDate}, "ts_code,trade_date,close"},
		{"us_adj_factor", "美股数据", "美股复权因子", "120积分",
		...
		// 财富管理（2）
		// ══════════════════════════════════════════════════════════════
		{"fund_sales_share", "财富管理", "各渠道基金销售占比", "2000积分",
			map[string]string{}, "quarter"},
		{"fund_sales_org", "财富管理", "销售机构基金销售保有", "2000积分",
			map[string]string{}, "quarter"},
	}
}
